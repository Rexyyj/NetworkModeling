...
class MMm_sys:
    ...
    def arrival_process(self, environment):
        while True:
            ...
            for key in self.mMMm.keys():
                if len(self.mMMm[key]) < self.queueSize:
                    cl = Client(self.TYPE1, environment.now)
                    self.mMMm[key].append(cl)
                    self.users += 1
                    forward_flag=0
                    break
            ...
            for key in self.mMMm.keys():
                if len(self.mMMm[key])==1 and self.BusyServer[key] is False:
                    cli = self.mMMm[key].pop(0)
                    self.BusyServer[key]=True
                    service_time= random.expovariate(1.0 / self.SERVICE)
                    environment.process(self.departure_process(environment, service_time, self.mMMm[key], cli, key))
                    break
